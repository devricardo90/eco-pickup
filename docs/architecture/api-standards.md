...
- scheduling action is available through `PATCH /api/v1/admin/pickup-requests/{id}/scheduling`
- scheduling persists `confirmedPickupWindowStartUtc` and `confirmedPickupWindowEndUtc`
- scheduling is currently allowed only when the request is `quoted`
- scheduling transitions the request to `scheduled`
- each scheduling action stores a status-history entry with actor, previous status, next status and optional note
- payment actions remain out of scope
